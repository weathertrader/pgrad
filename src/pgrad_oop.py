...
###############################################################################
class SfcStn:
    def __init__(self, project_name, use_stn, print_stn_info):
        self.stn_metadata = {}
        ...
        self.stn_name = stn_read_df['stn_name'].values
        self.stn_id   = stn_read_df['stn_id'].values
        self.stn_lon  = stn_read_df['stn_lon'].values
        self.stn_lat  = stn_read_df['stn_lat'].values
        self.stn_ele  = stn_read_df['stn_ele'].values
        self.stn_obs_hgt = stn_read_df['stn_obs_hgt'].values
        self.stn_mnet_id = stn_read_df['stn_mnet_id'].values
        self.stn_tmzn = stn_read_df['stn_tmzn'].values
        self.stn_dt_start = stn_read_df['stn_datetime_start'].values
        self.stn_dt_end = stn_read_df['stn_datetime_end'].values
        self.stn_state = stn_read_df['stn_state'].values
        self.stn_county = stn_read_df['stn_county'].values
        self.n_stn = len(self.stn_lat)
        del stn_read_df
        
        print ('use_stn is %s ' % (use_stn)) 
        if (use_stn != 'all'): 
            if   ('mnet' in use_stn):  
                mnet_id_list = use_stn.split('=')[-1].split(',')
                n_mnet_id = len(mnet_id_list)            
                mnet_id_int = []
                for nm in range(0, n_mnet_id): 
            ...
            self.stn_name           = self.stn_name          [use_stn]
            self.stn_id             = self.stn_id            [use_stn] 
            self.stn_lon            = self.stn_lon           [use_stn] 
            self.stn_lat            = self.stn_lat           [use_stn] 
            self.stn_ele            = self.stn_ele           [use_stn] 
            self.stn_obs_hgt        = self.stn_obs_hgt       [use_stn]   
            self.stn_mnet_id        = self.stn_mnet_id       [use_stn]
            self.stn_tmzn           = self.stn_tmzn          [use_stn]
            self.stn_datetime_start = self.stn_datetime_start[use_stn] 
            self.stn_datetime_end   = self.stn_datetime_end  [use_stn] 
            self.stn_state          = self.stn_state         [use_stn] 
            self.stn_county         = self.stn_county        [use_stn] 
            self.stn_cwa            = self.stn_cwa           [use_stn] 
            self.stn_notes          = self.stn_notes         [use_stn] 
    
        self.n_stn = len(self.stn_lat)
        if (print_stn_info): 
            print ('station infomation list ') 
            # for s,stn_name_temp in enumerate(stn_name):          
            for s in range(0, self.n_stn): 
               #print ('  stn_id %s lon %s lat %s ele %s obs_hgt %s ' % (stn_id[s], stn_lon[s], stn_lat[s], stn_ele[s], stn_obs_hgt[s])) 
               print ('  %s, lon %2.2f, lat %2.2f, ele %6.2f obs_hgt %2.1f, %s ' % (self.stn_id[s], self.stn_lon[s], self.stn_lat[s], self.stn_ele[s], self.stn_obs_hgt[s], self.stn_name[s])) 
...
class WallClockTime:
    def __init__(self):
        ...
        self.dt_start_utc =  dt.utcnow()
        yy_temp = self.dt_start_utc.year        
        if   (yy_temp == 2018):
            ...
            print('ERROR - DST not defined for this year')
            sys.exit()
        if ((self.dt_start_utc >= dt_dst_start) and (self.dt_start_utc <= dt_dst_end)):
            self.utc_conversion, self.time_zone_label = 7, 'PDT'
        else:
            self.utc_conversion, self.time_zone_label = 8, 'PST'
 
        self.wallClockTime.dt_start_lt = self.dt_start_utc - td(hours=self.utc_conversion)
...
class Models:
    def __init__(self):
        self.model_forecast_horizon = {}
        self.model_name_list = ['gfs', 'nam', 'hrrr']
        self.model_forecast_horizon['gfs'] = 24*10+1
        self.model_forecast_horizon['nam'] = 60+1
        self.model_forecast_horizon['hrrr'] = 36+1 
        self.update_frequency_hrs = {}
        self.update_frequency_hrs['gfs'] = 6
        self.update_frequency_hrs['nam'] = 6
        self.update_frequency_hrs['hrrr'] = 6
        self.dt_init_expected = None

    '''get expected most recent forecast based on wall clock time  '''
    def model_init(self, model_name, update_frequency_hrs):
        ...
        if   (update_frequency_hrs == 1):
            if (dt_utc.minute < 21):
                dt_temp = dt_utc - td(hours=2)
            else: 
                dt_temp = dt_utc - td(hours=1)
            dt_temp = dt(dt_temp.year, dt_temp.month, dt_temp.day, dt_temp.hour, 0, 0)
        elif (update_frequency_hrs == 6):
            dt_temp = dt_utc - td(hours=1) # was 2
            if    (dt_temp.hour < 6):
                hour_to_use = 0
            elif ((dt_temp.hour >=  6) & (dt_temp.hour < 12)):
                hour_to_use = 6
            elif ((dt_temp.hour >= 12) & (dt_temp.hour < 18)):
                hour_to_use = 12
            elif (dt_temp.hour >= 18):
                hour_to_use = 18
            dt_temp = dt(dt_temp.year, dt_temp.month, dt_temp.day, hour_to_use, 0, 0)
            ...
            if    (dt_temp.hour < 12):
                ...
                hour_to_use = 12
            self.dt_init_expected = dt(dt_temp.year, dt_temp.month, dt_temp.day, hour_to_use, 0, 0)
        else: 
            print      ('  ERROR: update_frequency not supported')
            
        #dt_init_expected -= td(hours=6)

###############################################################################    
def create_log_file(log_name_full_file_path, wallClockTime):
    ...
    print      ('instantiate_logger at %s %s ' % (wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M'), time_zone_label))
    logger.info('instantiate_logger at %s %s ' % (wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M'), time_zone_label))
    return logger


###############################################################################
def close_logger(logger, process_name, wallClockTime):
    dt_end_lt = dt.utcnow() - td(hours=wallClockTime.utc_conversion)
    time_delta_minutes = (dt_end_lt - wallClockTime.dt_start_lt).seconds/60.0
    print      ('###############################################################################') 
    logger.info('###############################################################################') 
    print      ('total_time %s %6.2f minutes, %s - %s [%s]  ' % (process_name, time_delta_minutes, wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M'), dt_end_lt.strftime('%Y-%m-%d_%H-%M'), wallClockTime.time_zone_label))
    logger.info('total_time %s %6.2f minutes, %s - %s [%s]  ' % (process_name, time_delta_minutes, wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M'), dt_end_lt.strftime('%Y-%m-%d_%H-%M'), wallClockTime.time_zone_label))    
...
###############################################################################
def process_grib_data(sfcStn, model_name, dt_init, forecast_horizon_hr, bucket_name):
    ...
    for n, file_temp in enumerate(file_list): 
        ...
        if not initial_read: 
            ...
            print_flag = True
            (j_loc_s, i_loc_s) = map_stn_locations_to_grid_locations(logger, sfcStn, lon_2d, lat_2d, print_flag)
            initial_read = True
        ...
        #np.shape(p1_sfc_stn)
        p1_sfc_stn = np.zeros([sfcStn.n_stn], dtype=float)
        p2_sfc_stn = np.zeros([sfcStn.n_stn], dtype=float)
        for s in range(0, sfcStn.n_stn):
        ...
        p_sfc_data = [sfcStn.stn_id, p1_sfc_stn, p2_sfc_stn]
...
###############################################################################
def process_csv_data(sfcStn, model_name, dt_init, forecast_horizon_hr, bucket_name):
    ...
    if os.path.isfile(p_sfc1_master_file): # read master
        ...
        p_sfc1_hr_s = np.full([forecast_horizon_hr, sfcStn.n_stn], np.nan, dtype=object)
        p_sfc2_hr_s = np.full([forecast_horizon_hr, sfcStn.n_stn], np.nan, dtype=object)
    ...
    p_sfc1_hr_s_df = pd.DataFrame(p_sfc1_hr_s, index=dt_axis, columns=sfcStn.stn_id) 
    p_sfc1_hr_s_df.to_csv(p_sfc1_master_file) 
    p_sfc2_hr_s_df = pd.DataFrame(p_sfc2_hr_s, index=dt_axis, columns=sfcStn.stn_id) 
...
###############################################################################
def calc_pgrad(sfcStn, stn_id_pair_list_to_plot, model_name, dt_init, bucket_name):
    ...
    #model_name = 'hrrr'
    #for model_name in models.model_name_list:
    ...
    if not os.path.isfile(p_sfc1_master_file): 
        ...
        for s, stn_id_pair in enumerate(stn_id_pair_list_to_plot):
            ...
            s_id1 = list(sfcStn.stn_id).index(stn_id1)
            s_id2 = list(sfcStn.stn_id).index(stn_id2)
            p_sfc1_diff_hr_s[:,s] = p_sfc1_hr_s_df[sfcStn.stn_id[s_id1]] - p_sfc1_hr_s_df[sfcStn.stn_id[s_id2]]
            p_sfc2_diff_hr_s[:,s] = p_sfc2_hr_s_df[sfcStn.stn_id[s_id1]] - p_sfc2_hr_s_df[sfcStn.stn_id[s_id2]]
            #column_str[s1*sfcStn.n_stn+s2] = sfcStn.stn_id[s1]+'-'+sfcStn.stn_id[s2]
...
###############################################################################
def plot_data(sfcStn, models.model_name_list, dt_init_expected, forecast_horizon_hr,  wallClockTime.dt_start_lt, utc_conversion, time_zone_label, stn_id_pair_list_to_plot, bucket_name):
    ...
    while dt_init <= dt_init_expected:
        ...
        model_name = 'gfs'
        m = 0
        for m, model_name in enumerate(models.model_name_list):
            ...
            if not os.path.isfile(p_sfc1_diff_master_file): 
                ...
                try: 
                    ...
                    if not result: # no existing entry 
                        ...
                        #        "dt_proc_lt": wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M')}
                        data = {"model": model_name,
                                "dt_init": dt_init, 
                                "hrs_avail": n_hrs_found,
                                "dt_proc_lt": wallClockTime.dt_start_lt}
                        ...
                        if n_hrs_found > n_hrs_prev:
                            ...
                            #                 'dt_init':dt_init},
                            #                {"$set":{"hrs_avail":n_hrs_found, "dt_proc_lt": wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M')}})
                            coll.update_one({'model':model_name, 
                                             'dt_init':dt_init},
                                            {"$set":{"hrs_avail":n_hrs_found, "dt_proc_lt": wallClockTime.dt_start_lt}})
    ...
    for s,stn_id_pair in enumerate(stn_id_pair_list_to_plot):
        ...
        if stn_id_pair in stn_id_pair_list_to_plot:
            ...
            for d, n_days in enumerate(n_days_list):
                ...
                plt.plot([wallClockTime.dt_start_lt, wallClockTime.dt_start_lt], [y_min, y_max], 'y', linestyle='--', linewidth=1.0, marker='o', markersize=0, markeredgecolor='k') 
                ...
                plt.title('$\Delta$ slp %s, %s Z init, updated: %s %s ' % (stn_id_pair, dt_init_list[i].strftime('%Y-%m-%d_%H'), wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M'), time_zone_label), \
                ...
                filename = 'del_slp_all_model_'+stn_id_pair+'_'+dt_init_list[i].strftime('%Y-%m-%d_%H')+'_'+str(n_days)+'_'+wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H')+'_'+time_zone_label+'.png' 
            ...
            for m, model_name in enumerate(models.model_name_list):
                for d, n_days in enumerate(n_days_list):
                    if (model_name == 'hrrr' and n_days == 2) or (model_name == 'nam' and n_days == 3) or (model_name == 'gfs' and n_days == 8):
                        ...
                        plt.xlabel('date time ['+time_zone_label+']',fontsize=size_font,labelpad=00)
                        plt.ylabel('$\Delta$ slp [mb]',fontsize=size_font,labelpad=20)                      
                        plt.title('$\Delta$ slp %s, %s model, updated: %s %s' % (stn_id_pair, model_name.upper(), wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M'), time_zone_label), \
                          fontsize=size_font+2, x=0.5, y=1.01)                     
                        plt.show() 
                        filename = 'del_slp_all_init_'+stn_id_pair+'_'+model_name+'_'+str(n_days)+'.png' 
                        plot_name = os.path.join('images',filename)
                        plt.savefig(plot_name) 
                        filename = 'del_slp_all_init_'+stn_id_pair+'_'+model_name+'_'+str(n_days)+'_'+wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H')+'_'+time_zone_label+'.png' 
                        plot_name = os.path.join('images', 'archive', filename)
                        plt.savefig(plot_name) 
...
###############################################################################
def obs_historical_download(sfcStn, stn_list_to_use, utc_conversion, time_zone_label, bucket_name):
    ...
    for s in range(0, sfcStn.n_stn): 
        #if ('SJS' in sfcStn.stn_name[s]):
        #if ('JBG' in sfcStn.stn_id[s]):
        print      ('  processing s %s, s = %s of %s ' % (sfcStn.stn_id[s], s, sfcStn.n_stn))  
        logger.info('  processing s %s, s = %s of %s ' % (sfcStn.stn_id[s], s, sfcStn.n_stn))  
        #for d in range(0, n_days, 1):
        if (sfcStn.stn_id[s] in stn_list_to_use):
            ...
            while yy <= dt.utcnow().year:
                ...
                file_name_stn_write = os.path.join(dir_sfc_obs_historical, 'stn_obs_'+sfcStn.stn_id[s]+'_'+str(yy)+'.csv') 
                if os.path.isfile(file_name_stn_write):
                    print      ('      skipping, file exists already')
                    logger.info('      skipping, file exists already')
                else:                
                    # url_temp = url_base+str(sfcStn.stn_id[s])+'&start='+dt_temp.strftime('%Y%m%d0000')+'&end='+dt_temp.strftime('%Y%m%d2359')+'&token='+mesowest_token+'&obtimezone=utc'+'&timeformat=%Y-%m-%d_%H-%M' 
                    # url_temp = url_base+str(sfcStn.stn_id[s])+'&start='+dt_start.strftime('%Y%m%d0000')+'&end='+dt_end.strftime('%Y%m%d2359')+'&token='+mesowest_token+'&obtimezone=utc'+'&timeformat=%Y-%m-%d_%H-%M' 
                    # slp and alt 
                    #url_temp = url_base+str(sfcStn.stn_id[s])+'&start='+dt_start.strftime('%Y%m%d0000')+'&end='+dt_end.strftime('%Y%m%d2359')+'&vars=sea_level_pressure,altimeter&token='+mesowest_token+'&obtimezone=utc'+'&timeformat=%Y-%m-%d_%H-%M' 
                    # slp only 
                    # url_temp = url_base+str(sfcStn.stn_id[s])+'&start='+dt_start.strftime('%Y%m%d0000')+'&end='+dt_end.strftime('%Y%m%d2359')+'&vars=sea_level_pressure&token='+mesowest_token+'&obtimezone=utc'+'&timeformat=%Y-%m-%d_%H-%M' 
                    # alt only
                    url_temp = url_base+str(sfcStn.stn_id[s])+'&start='+dt_start.strftime('%Y%m%d0000')+'&end='+dt_end.strftime('%Y%m%d2359')+'&vars=altimeter&token='+mesowest_token+'&obtimezone=utc'+'&timeformat=%Y-%m-%d_%H-%M' 
                    ...
                    if (len(data_parse) == 0): # station contains no data 
                        ...
                        #file_name_stn_write = os.path.join(dir_data_ingest, 'stn_obs_'+sfcStn.stn_id[s]+'_'+dt_temp.strftime('%Y-%m-%d')+'.csv') 
...
###############################################################################
def obs_historical_process(sfcStn, utc_conversion, time_zone_label, stn_id_pair_list_to_plot, bucket_name):
    ...
    #slp_obs_5min_s  = np.full([n_5min, sfcStn.n_stn], np.nan, dtype='float') 
    alt_obs_5min_s  = np.full([n_5min, sfcStn.n_stn], np.nan, dtype='float') 
    ...
    for s in range(0, sfcStn.n_stn):
        ...
        print      ('  processing s %s, s = %s of %s ' % (sfcStn.stn_id[s], s, sfcStn.n_stn))  
        logger.info('  processing s %s, s = %s of %s ' % (sfcStn.stn_id[s], s, sfcStn.n_stn))  
        file_name_list = glob.glob(os.path.join(dir_sfc_obs_historical,'stn_obs*'+sfcStn.stn_id[s]+'*csv'))    
        file_name_list.sort()
        n_files = len(file_name_list)
        print      ('  stn_id %s, n_files %s ' % (sfcStn.stn_id[s], n_files)) 
        logger.info('  stn_id %s, n_files %s ' % (sfcStn.stn_id[s], n_files)) 
        f_count = 0
        if (n_files > 0): 
    ...
    for s, stn_id_pair in enumerate(stn_id_pair_list_to_plot):
        ...
        #sfcStn.stn_id[s_id1]
        #sfcStn.stn_id[s_id2]
        #pres_diff_5min_s[:,s1*sfcStn.n_stn+s2] = pres_obs_5min_s[:,s1] - pres_obs_5min_s[:,s2] 
        alt_diff_5min_s [:,s] = alt_obs_5min_s[:,s_id1] -  alt_obs_5min_s[:,s_id2] 
        #slp_diff_5min_s [:,s1*sfcStn.n_stn+s2] =  slp_obs_5min_s[:,s1] -  slp_obs_5min_s[:,s2] 
        #stn_id_pair_list[s1*sfcStn.n_stn+s2] = sfcStn.stn_id[s1]+'-'+sfcStn.stn_id[s2]
    # del slp_obs_5min_s
...
if __name__ == "__main__":
    ...
    print(wallClockTime.wallClockTime.dt_start_lt)
    print(wallClockTime.dt_start_utc)
    print(wallClockTime.utc_conversion)
    print(wallClockTime.time_zone_label)
    
    models = Models()
    update_frequency_hrs = models.update_frequency_hrs[model_name]
    model_forecast_horizon = models.model_forecast_horizon[model_name]

    # sanitize inputs
    if model_name not in models.model_name_list:
        print('ERROR model_name %s not supported' %(model_name))
        sys.exit()
    if process_name not in ['debug', 'cleanup', 'download', 'process_grib', 'process_csv', 'calc_pgrad', 'plot_data', 'obs_historical_download', 'obs_historical_process']:
        print('ERROR process_name %s not supported' %(process_name))
        sys.exit()
    ...
    if  model_name is not None:
        log_file_name = 'log_'+process_name+'_'+model_name+'_'+wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M')+'.txt' 
    else:
        log_file_name = 'log_'+process_name+'_'+wallClockTime.dt_start_lt.strftime('%Y-%m-%d_%H-%M')+'.txt' 
    #if  process_name in ['download', 'process_grib', 'process_csv', 'calc_pgrad']:
    log_name_full_file_path = os.path.join('logs', log_file_name) 
    logger = create_log_file(log_name_full_file_path, wallClockTime) 
    ...
    print      ('  expecting %s %s Z to be available ' % (model_name, models.dt_init_expected.strftime('%Y-%m-%d %H')))
    logger.info('  expecting %s %s Z to be available ' % (model_name, models.dt_init_expected.strftime('%Y-%m-%d %H')))
   
    project_name = 'pgrad'    
    use_stn = 'all'
    print_stn_info = False
    # (sfcStn) = read_stn_metadata_from_csv(stn_metadata_file_name, use_stn, print_stn_info)
    print('initialize SfcStn class begin')
    sfcStn = SfcStn(project_name, use_stn, print_stn_info)
    print('initialize SfcStn class end')
    #print(sfcStn.stn_id)
    #print(sfcStn.stn_lat)
    ...
    dt_init =  models.dt_init_expected - td(hours=hours_to_backfill)
    if  process_name in ['download', 'process_grib', 'process_csv', 'calc_pgrad']:
        while dt_init <= dt_init_expected: 
            print      ('***************************')
            logger.info('***************************')
            ...
            if   process_name == 'download':
                download_data(model_name, models, bucket_name)
            elif process_name == 'process_grib':
                process_grib_data(sfcStn, model_name, models, bucket_name)
            elif process_name == 'process_csv':
                process_csv_data(sfcStn, model_name, models, bucket_name)
            elif process_name == 'calc_pgrad':                
                calc_pgrad(sfcStn, stn_id_pair_list_to_plot, model_name, models, bucket_name)
            dt_init += td(hours=update_frequency_hrs)

    dt_init = dt_init_expected
    if process_name == 'plot_data':
        #plot_data(model_name_list, dt_init, forecast_horizon_hr=120+1, bucket_name)
        plot_data(sfcStn, models, dt_init, 241, wallClockTime, stn_id_pair_list_to_plot, bucket_name)
    if process_name == 'obs_historical_download':
        obs_historical_download(sfcStn, stn_list_to_use, wallClockTime, bucket_name)
    if process_name == 'obs_historical_process':
        obs_historical_process(sfcStn, wallClockTime, stn_id_pair_list_to_plot, bucket_name)
    ...
    close_logger(logger, process_name, wallClockTime) 
